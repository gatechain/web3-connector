...
class MetaMaskWallet extends AbstractWallet {
  ...
  async detectProvider() {
    try {
      ...
      const provider = provider$1?.providers?.length ? provider$1?.providers.find(p => p.isMetaMask) ?? provider$1.providers[0] : provider$1;
  ...
  async autoConnect() {
    ...
    if (!provider) return;
    try {
      const [chainId, accounts] = await Promise.all([this.provider.request({
        method: 'eth_chainId'
      }), this.provider.request({
        method: 'eth_requestAccounts'
      })]);

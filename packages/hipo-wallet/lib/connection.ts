...
class MetaMaskConnector {
  ...
  public static getInstance() {
    if (!this.instance) {
      this.instance = initializeConnector<MetaMask>((actions) => new MetaMask({ actions }))
    }
...
class CoinbaseWalletConnector {
  private constructor() { }
  private static instance: ReturnType<typeof initializeConnector<CoinbaseWallet>>
  public static getInstance(URLS: URLMap) {
    if (!this.instance) {
      this.instance = initializeConnector<CoinbaseWallet>((actions) => new CoinbaseWallet({
        actions,
        options: {
          url: URLS[1][0],
          appName: 'web3-react',
...
class WalletConnectConnector {
  private constructor() { }
  private static instance: ReturnType<typeof initializeConnector<WalletConnect>>
  public static getInstance(URLS: URLMap) {
    if (!this.instance) {
      this.instance = initializeConnector<WalletConnect>((actions) => new WalletConnect({ actions, options: { rpc: URLS } }))
    }
    return this.instance
  }
}
